    ...
    async def fetch_new_comics(self, days: int = 7):
        ...
        for fetch_date in random_dates:
            try:
                ...
                comic_data = await self.comic_service.fetch_calvin_and_hobbes(fetch_datetime)
                if comic_data:
    ...
    async def run_scheduler(self):
        ...
        while True:
            try:
                ...
                logger.info(f"Next post scheduled for {datetime.now() + timedelta(days=self.DAYS_BETWEEN_POSTS)}")
